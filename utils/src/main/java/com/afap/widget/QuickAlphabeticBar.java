...
public class QuickAlphabeticBar extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!initFlag) {// 初始化一次尺寸
			...
			mCharHeight = (int) (mSingleHeight * 0.75);
			initFlag = true;
		}
		if (showBkg) {// 按条件绘制背景
		...
		for (int i = 0; i < letters.length; i++) {
			...
			if (i == choose) {
				paint.setColor(Color.parseColor("#f1517d"));
			}
	...
	private void showPopup(int item) {
		if (mPopupWindow == null) {// 初始化弹出窗口
			mPopupText = new TextView(getContext());
			mPopupText
					.setBackgroundResource(R.drawable.choosecity_quickalpbar_pop_bg);
			mPopupText.setTextColor(Color.parseColor("#f1517d"));
